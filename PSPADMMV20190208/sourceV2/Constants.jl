...
if test_5D == 1
    ...
    global  Profits = 20.8184e6 #22.613e6#
...
#    global  sSCALE = 0.5*10*[130 256  203  327  84] #0.5*1e2*[12, 6, 8, 5, 20]  #1e2/2#1e1   # scale up supplyR [1, 1, 1, 1, 1]#1e1*[12, 13, 12, 8, 12]#
    global  sSCALE = 10*[130 256  203  327  84]/1000 #0.5*1e2*[12, 6, 8, 5, 20]  #1e2/2#1e1   # scale up supplyR [1, 1, 1, 1, 1]#1e1*[12, 13, 12, 8, 12]#
    global  cSCALE = 1/1000 #0.5*1e2*[12, 6, 8, 5, 20]  #1e2/2#1e1   # scale up supplyR [1, 1, 1, 1, 1]#1e1*[12, 13, 12, 8, 12]#
    ...
    global  sSCALE = 10*[130, 220, 250, 250, 150]/1000#1e2*[5, 5, 5, 9, 5]    # scale up supplyR
    global  cSCALE = 1/1000 #0.5*1e2*[12, 6, 8, 5, 20]  #1e2/2#1e1   # scale up supplyR [1, 1, 1, 1, 1]#1e1*[12, 13, 12, 8, 12]#
...
global  NOptions = 5                # number of options in the price menu
global  NConsumers = 1000
global  NScenarios = 10           # number of scenarios for renewable production

global NPartitions = 1000
# it is not necessary to use 1000 partitions for the first breakpoint
global NPartitionsChosen = 100
