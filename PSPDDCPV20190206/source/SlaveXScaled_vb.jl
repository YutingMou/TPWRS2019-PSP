...
function SolveSlaveX(inputSlaveX)
    ...
    @variables slaveX begin
        ...
        #u_consumer[ConsumersSet, OptionsSet], Bin   # binary
        0 <= vb <= Vmax # the first valuation vrakpoint
        supplyR_aux[TimeSetX, PartitionsSet] >= 0
        0 <= vb_aux[PartitionsSet] <= Vmax  # only the first breakpoint has many partitions
        u_partition[PartitionsSet], Bin # partition indicator of the first breakpoint
        w_supplyR_vb[TimeSetX] >= 0# supplyR*vb for the first breakpoint
    end

    #println("variables defined at ", now())
    #flush(STDOUT)
   # use the fixed breakpoint, assume all consumers are served
   @constraints(slaveX, begin
    ...
    for t = TimeSetX
        @constraints(slaveX, begin
            w_supplyR_vb[t] >= sum(supplyR_aux[t,partition]*vb_LB[partition] + 0*vb_aux[partition] -
                0*vb_LB[partition]*u_partition[partition] for partition = PartitionsSet)
            w_supplyR_vb[t] >= sum(supplyR_aux[t,partition]*vb_UB[partition] + supplyR_U*vb_aux[partition] -
                supplyR_U*vb_UB[partition]*u_partition[partition] for partition = PartitionsSet)
            w_supplyR_vb[t] <= sum(supplyR_aux[t,partition]*vb_LB[partition] + supplyR_U*vb_aux[partition] -
                supplyR_U*vb_LB[partition]*u_partition[partition] for partition = PartitionsSet)
            w_supplyR_vb[t] <= sum(supplyR_aux[t,partition]*vb_UB[partition] + 0*vb_aux[partition] -
                0*vb_UB[partition]*u_partition[partition] for partition = PartitionsSet)
        end)
    end

    # the chosen vb should lie in this partition
    for partition = PartitionsSet
        @constraints(slaveX, begin
            vb_aux[partition] >= vb_LB[partition]*u_partition[partition]
            vb_aux[partition] <= vb_UB[partition]*u_partition[partition]
        end)
    end
    # only one partition is chosen
    @constraints(slaveX, begin
        sum(u_partition[partition] for partition = PartitionsSet) == 1
        sum(vb_aux[partition] for partition = PartitionsSet) == vb
    end)

    # limit on supplyR_aux
    for t = TimeSetX, partition = PartitionsSet
        @constraints(slaveX, begin
            supplyR_aux[t, partition] <= supplyR_U*u_partition[partition]
        end)
    end
    ...
    for t = TimeSetX
        @constraints(slaveX, begin
            sum(supplyR_aux[t, partition] for partition = PartitionsSet) == supplyR[1, t]
        end)
    end
    ...
    if status_slaveX == :Optimal
        ...
        if false #niterationsDD < MaxIterationDD
            ...
            vb_sol = getvalue(vb)
            u_partition_sol = getvalue(u_partition[:])
            u_partition_chosen = sum(u_partition_sol[partition]*partition for partition = PartitionsSet)
            ...
            return  subproblemIdx, totalCost_current_sol, supplyR_current_sol, u_sol,
                    ...
                    p_prod_sol, loadShedding_sol, renewableShedding_sol, vb_sol,
                    u_partition_chosen, currentslaveXobjval
